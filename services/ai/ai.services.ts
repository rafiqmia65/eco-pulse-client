...
export async function getAIRecommendationsAction(): Promise<
  ApiResponse<IAIRecommendation[]>
> {
  try {
    const res = await httpClient.get<IAIRecommendation[]>(
...
): Promise<ApiResponse<IAIChatResponse>> {
  try {
    const res = await httpClient.post<IAIChatResponse>("/api/v1/ai/chat", {
      message,
      conversationId,
    });
    return res;
 ...
 */
export async function analyzeIdeaAction(
  ideaId: string,
): Promise<ApiResponse<IAIAnalysisReport>> {
  try {
    const res = await httpClient.get<IAIAnalysisReport>(
      `/api/v1/ai/analyze/${ideaId}`,
    );
    return res;
  } catch (error: any) {
    return (
...
export async function getAIConversationsAction(): Promise<
  ApiResponse<IAIConversation[]>
> {
  try {
    const res = await httpClient.get<IAIConversation[]>(
...
export async function getAIConversationMessagesAction(
  conversationId: string,
): Promise<ApiResponse<IAIMessage[]>> {
  try {
    const res = await httpClient.get<IAIMessage[]>(
